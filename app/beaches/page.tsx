...
export default function BeachesPage() {
  return <main className="simple-page directory-page">
    ...
    <section className="directory-grid">{beaches.map((beach) => <Link className="directory-card" href={beachPath(beach)} key={beach.slug}><span>{beach.region}</span><h2>{beach.name}</h2><p>{beach.description}</p><b>Live conditions →</b></Link>)}</section>
  </main>;
}
